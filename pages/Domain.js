...
const DomainPage = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get("https://genshin.jmp.blue/domains");
  ...
  const Card = ({ domain }) => {
    ...
    const navigateToDetail = () => {
      navigation.navigate("DetailDomainPage", {
        name: domain,
  ...
  return (
    <View style={styles.container}>
      <View style={styles.headerContainer}>
        <Text style={styles.headerTitle}>List of Domains:</Text>
        <Text>Klik salah satu card untuk masuk ke detail</Text>
      </View>
      <FlatList data={domainsData} keyExtractor={(item, index) => index.toString()} renderItem={({ item }) => <Card domain={item} />} contentContainerStyle={styles.listContainer} />
    </View>
  );
};
