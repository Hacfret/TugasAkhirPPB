...
const CharacterPage = () => {
  ...
  const Card = ({ character }) => {
    ...
    const navigateToDetail = () => {
      navigation.navigate("DetailCharacterPage", {
        name: character,
  ...
  return (
    <View style={styles.container}>
      <View style={styles.headerContainer}>
        <Text style={styles.headerTitle}>List of Characters:</Text>
        <Text>Klik salah satu card untuk masuk ke detail</Text>
      </View>
      <FlatList data={charactersData} keyExtractor={(item, index) => index.toString()} renderItem={({ item }) => <Card character={item} />} contentContainerStyle={styles.listContainer} />
    </View>
  );
};
